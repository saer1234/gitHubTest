...
const getIncome= async(req,res,next)=>{
   ...
   const [kitchen]=await connection.query("SELECT * FROM income INNER JOIN items ON income.item_id=items.item_id WHERE date_in BETWEEN ? AND ? AND kitchen_id=? ORDER BY date_in",[date_in,date_out,id]);
   ...
   if(data.length==0)
    {
        ...
        for(let x of data)
        {
            const [select] = await connection.query("SELECT * FROM income WHERE income.item_id=? AND date_in=? AND income.kitchen_id=?",[x.item_id,x.date_in,x.kitchen_id]);
            if(select.length>0){
                const [result]= await connection.query("UPDATE income SET amount=? WHERE income.item_id=? AND date_in=? AND income.kitchen_id=?",[parseFloat(select[0].amount)+parseFloat(x.amount),x.item_id,x.date_in,x.kitchen_id]);
            }else{
                const [result]= await connection.query("INSERT INTO income(amount,date_in,item_id,kitchen_id) values(?,?,?,?)",[x.amount,x.date_in,x.item_id,x.kitchen_id]);
            }
        }
...
const updateincome=async(req,res,next)=>{
    ...
    const [kitchen]= await connection.query("SELECT * FROM income WHERE income_id=?",[id]);
    if(kitchen.length==0){
        return res.status(401).json({success:false,message:"income id not fouend"});
    } 
    const [result] = await connection.query("UPDATE income SET amount=?, date_in=? WHERE income_id=?",[data.amount,data.date_in,id]);
...
const deleteincome=async(req,res,next)=>{
    ...
    const [kitchen]= await connection.query("SELECT * FROM income WHERE income.income_id=?",[id]);
    if(kitchen.length==0){
        return res.status(401).json({success:false,message:"id of in stock not founded"});
    } 
    const [result] = await connection.query("DELETE FROM income WHERE income_id=?",[id]);
    if(result.affectedRows===0)
        return res.status(404).json({success:false,message:"YOu can't modify income "});
        successResponse(res,kitchen);
}
const getTotalAmont= async(req,res,next)=>{
    ...
    const [result] = await connection.query("SELECT * FROM income INNER JOIN items ON items.item_id=income.item_id WHERE  date_in BETWEEN ? AND ? AND kitchen_id=? ORDER BY date_in",[date_in,date_out,id]);
    const listItems= [];
    for(let x of result){
...
module.exports={getTotalAmont,updateincome,deleteincome,getIncome,createIncome};