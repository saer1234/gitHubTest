...
@Component({
  ...
  templateUrl: './addqanutity.component.html',
  styleUrl: './addqanutity.component.css'
})
export class AddqanutityComponent implements OnInit ,AfterViewInit{
  ...
  ListItems:{item_id:number,item_name:string,date:string}[]=[];
...
this.dataSouce1.filter=(event.target as HTMLInputElement).value.trim().toLowerCase();
  ...
  openDialogEdit(element:any){
      this.dialog.open(EditIncomeComponent,{data:{element:element,listITems:this.ListItems,listKitchen:this.ListKitchenNmae}});
  }
  DeleteItemsA(i:any){
    this.incomeService.deleteAmount(i.income_id).subscribe((response:any)=>{
      if(response.success==true){
        this.searchData();
